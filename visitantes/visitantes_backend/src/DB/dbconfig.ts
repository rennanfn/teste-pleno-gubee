...
export default {
  user: process.env.DB_DEV_USER_NAME_VISITANTES,
  password: process.env.DB_DEV_USER_PASSWORD_VISITANTES,
  connectString: process.env.DB_DEV_CONNECT_DESCRIPTOR_VISITANTES,
  // De acordo com a documentação do oracle db, quando aumetamos o poolMax é
  // bom aumentar a quantidade de thread que podem ser utilizadas tbm, isso é
  // feito criando uma variável de ambiente UV_THREADPOOL_SIZE no ambiente (esta no ecosystem do pm2)
  // https://github.com/oracle/node-oracledb/blob/node-oracledb-v1/doc/api.md#numberofthreads
  poolMin: 120,
  poolMax: 120,
  poolIncrement: 0,
};
